...
def log_transform(X_df):
    """
    ستون‌های نرخ‌محور (مثل Flow Bytes/s) توزیع بسیار کج و zero-inflated
    دارند: اکثر مقادیر نزدیک صفرند و فقط یک دنباله‌ی طولانی مقادیر بزرگ
    وجود دارد. در این حالت IQR این ستون‌ها بسیار کوچک می‌شود، و وقتی
    RobustScaler مقادیر را بر این IQR کوچک تقسیم می‌کند، حتی مقادیر عادی
    هم به اعداد بسیار بزرگ تبدیل می‌شوند (همان چیزی که در Loss ~1e12 و
    Threshold ~2.7e13 دیدیم).
    راه‌حل: قبل از Scale کردن یک log1p علامت‌دار روی داده اعمال می‌کنیم تا
    دامنه‌ی مقادیر بزرگ فشرده و توزیع متقارن‌تر شود. از np.sign استفاده
    می‌کنیم تا اگر مقدار منفی هم وجود داشت، تبدیل درست انجام شود.
    """
...
def analyze_feature_separation(train_data_raw, attack_dfs, constant_cols, feature_columns,
    ...
    print("\n================ تحلیل تفکیک‌پذیری ویژگی‌ها (مستقل از مدل) ================")
    for name, df in attack_dfs.items():
        ...
        print(f"\n{name} -- {top_n} ویژگی با بیشترین تفاوت نسبت به ترافیک نرمال (Cohen's d):")
