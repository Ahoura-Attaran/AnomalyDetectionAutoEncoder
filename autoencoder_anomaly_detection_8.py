...
def analyze_feature_separation(train_data_raw, attack_dfs, constant_cols, feature_columns,
    ...
    for name, df in attack_dfs.items():
        ...
        print(f"\n{name} -- {top_n} featuers with most differece with normal (Cohen's d):")
...
def compute_feature_weights(effect_sizes_per_attack, feature_columns, min_weight=1.0, max_weight=5.0):
    ...
    print("\n================ final featuer weight for Weighted MSE Loss ================")
...
def find_best_threshold(model, normal_errors, attack_dfs, scaler, constant_cols, feature_columns,
                         clip_lower, clip_upper, skewed_cols, feature_weights=None,
    ...
    for name, df in attack_dfs.items():
        ...
        errs = reconstruction_error(model, X_sample_scaled, feature_weights)
    ...
    print(f"Threshold کالیبره‌شده با F1 بهینه (روی نمونه‌ی کوچک کالیبراسیون): "
          f"{best_threshold:.4f}  |  F1 کالیبراسیون = {best_f1:.3f}")
...
def evaluate_model(model, threshold, scaler, constant_cols, feature_columns,
                    clip_lower, clip_upper, skewed_cols, feature_weights,
    ...
    errors = reconstruction_error(model, X_test_all, feature_weights)
...
def evaluate_per_attack_type(model, threshold, scaler, constant_cols, feature_columns,
                              clip_lower, clip_upper, skewed_cols, feature_weights,
                              test_normal_X, attack_dfs, model_name):
    normal_errors = reconstruction_error(model, test_normal_X, feature_weights)
    y_normal = np.zeros(len(normal_errors))

    print(f"\n================ تفکیک بر اساس نوع حمله: {model_name} ================")
    ...
    for name, df in attack_dfs.items():
        ...
        att_errors = reconstruction_error(model, X_att_scaled, feature_weights)
...
def main():
    ...
    print(f"تعداد ستون‌های کج که log1p روی آن‌ها اعمال می‌شود: {len(skewed_cols)} از {len(feature_columns)}")
    ...
    shallow_val_errors = reconstruction_error(shallow_model, X_val, feature_weights)
    ...
    print(f"(برای مقایسه: Threshold مبتنی بر صدک ۹۹ فقط از داده‌ی نرمال = {reference_percentile_threshold:.4f})")
    ...
    y_true_shallow, y_pred_shallow, errors_shallow = evaluate_model(
        ...
        clip_lower, clip_upper, skewed_cols, feature_weights, X_test_normal, attack_dfs, "Shallow Autoencoder"
    )
    ...
    deep_val_errors = reconstruction_error(deep_model, X_val, feature_weights)
    ...
    y_true_deep, y_pred_deep, errors_deep = evaluate_model(
        ...
        clip_lower, clip_upper, skewed_cols, feature_weights, X_test_normal, attack_dfs, "Deep Autoencoder"
    )
    ...
    print("\n================ جدول مقایسه‌ی نهایی: Shallow vs Deep (بر اساس Recall) ================")
    print(comparison[[
